...
class SuggestionServiceClient implements SuggestionServiceIf
{
    ...
    public function send_execute(SuggestParams $suggestParams)
    {
        ...
        if ($bin_accel) {
            thrift_protocol_write_binary($this->output_, 'execute', TMessageType::CALL, $args, $this->seqid_,
    ...
    public function recv_execute()
    {
        ...
        if ($bin_accel) {
            $result = thrift_protocol_read_binary($this->input_,
                '\OpenSearch\Generated\Suggestion\SuggestionService_execute_result', $this->input_->isStrictRead());
        } else {
            $rseqid = 0;
            $fname = null;
            $mtype = 0;
    ...
    public function send_search()
    {
        ...
        $bin_accel = ($this->output_ instanceof TBinaryProtocolAccelerated) && function_exists('thrift_protocol_write_binary');
        if ($bin_accel) {
            thrift_protocol_write_binary($this->output_, 'search', TMessageType::CALL, $args, $this->seqid_,
                $this->output_->isStrictWrite());
        } else {
    ...
    public function recv_search()
    {
        ...
        if ($bin_accel) {
            $result = thrift_protocol_read_binary($this->input_,
                '\OpenSearch\Generated\Suggestion\SuggestionService_search_result', $this->input_->isStrictRead());
        } else {
            $rseqid = 0;
            $fname = null;
            $mtype = 0;
